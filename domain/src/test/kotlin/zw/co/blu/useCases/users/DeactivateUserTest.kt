...
internal class DeactivateUserTest {
    ...
    private val output: User = User("", "", "", UserStatus.INACTIVE, role)
    ...
    fun `it should return successful response`() {
        ...
        coEvery { userServiceMock.deactivateUser(any()) } returns expected

        val sut = DeactivateUser(userServiceMock)

        runBlocking { assertEquals(expected, sut.invoke(input)) }

        coVerify { userServiceMock.deactivateUser(any()) }
    }
